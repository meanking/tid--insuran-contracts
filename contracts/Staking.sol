...
contract Staking is IStaking, Ownable, WeekManaged, Migratable, NonReentrancy, BaseRelayRecipient {
    ...
    struct PoolInfo {
        ...
        uint256 accRewardPerShare; // Accumulated UNO per share, times UNIT_PER_SHARE.
    }
    ...
    function migrate() external lock {
        ...
        IERC20(registry.unoToken()).transfer(address(migrateTo), userAmount);
    ...
    // View function to see pending UNO on frontend.
    ...
    function deposit(uint256 amount_) external lock {
        ...
        IERC20(registry.unoToken()).transferFrom(
            _msgSender(),
            address(this),
            amount_
        );
    ...
    function withdrawReady(address who_, uint256 index_) external lock {
        ...
        IERC20(registry.unoToken()).transfer(address(who_), request.amount);
    ...
    function claim() external lock {
        ...
        IERC20(registry.unoToken()).transfer(_msgSender(), rewardTotal);
    ...
    function finishPayout(uint16 assetIndex_, uint256 payoutId_) external {
        ...
        if (payoutInfo[assetIndex_][payoutId_].paid < payoutInfo[assetIndex_][payoutId_].total) {
            ...
            IERC20(registry.unoToken()).transferFrom(msg.sender,
                address(this),
        ...
        IERC20(registry.unoToken()).transfer(payoutInfo[assetIndex_][payoutId_].toAddress,
